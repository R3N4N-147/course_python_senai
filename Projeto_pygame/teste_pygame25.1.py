...
def resetar_jogo(): # V. 17
    ...
    global frutas_coletadas, proximo_alvo_fruta, tempo_fase, metas_batidas, timer_vitoria_fase
    # vidas_char = 3
    vidas_char = 5
    x_barril = largura
...
while True:
    ...
    # VERSAO 15
        ...
        if metas_batidas:
            ...
            if timer_vitoria_fase > 0:
                ...
                status_jogo = "AGUARDANDO_BOSS" # Mude para esse estado para destravar < --- V.25
    ...
    # Versão 25/26 - Lógica da Luta Final Corrigida
    elif status_jogo == "BOSS_LUTA":
        tela.blit(wallpaper_boss, (0, 0)) # Fundo fixo da luta
        timer_boss += 1
        keys = pygame.key.get_pressed()

        # 1. IA DO CHEFE (Movimento e Ataque)
        if status_boss == "MOVENDO":
            x_boss += vel_boss_x
            y_boss += vel_boss_y
            # Mantém o boss no lado direito da tela
            if x_boss <= 450 or x_boss >= largura - 100: vel_boss_x *= -1
            if y_boss <= 100 or y_boss >= altura - 250: vel_boss_y *= -1
            
            if timer_boss >= 180: # Ataca a cada 3 segundos
                status_boss = "PISCANDO"
                timer_boss = 0

        elif status_boss == "PISCANDO":
            if timer_boss >= 60: # Tempo de aviso (1 segundo)
                status_boss = "ATACANDO"
                timer_boss = 0

        elif status_boss == "ATACANDO":
            # Golpe: Explosão aparece no Luffy
            tela.blit(luffy_explosao, (x_lufy - 20, y_lufy - 20)) 
            if not luffy_no_ar and timer_boss == 1: # Dano apenas no 1º frame do golpe
                vidas_char -= 1
                x_lufy -= 50 # Empurrão
        ...
        # 2. CONTROLES DO LUFFY NA LUTA
        if keys[K_a] and x_lufy > 50: x_lufy -= 5
        if keys[K_d] and x_lufy < 400: x_lufy += 5
        ...
        # 3. DESENHO DOS PERSONAGENS (Com correção do piscar)
        # O Boss só some rápido se estiver "PISCANDO"
        ...
        txt_boss = fonte.render(f"BOSS: {vidas_boss}", True, (255, 0, 0))
        tela.blit(txt_boss, (largura - 150, 30))

        # Condição de derrota do Luffy
        if vidas_char <= 0:
            status_jogo = "DIGITANDO_NOME"
            resetar_jogo()
