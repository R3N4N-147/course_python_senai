...
# versão 26.py Janela de Vulnerabilidade do Chefe e Sistema de 5 Vidas (Boss). 2 horas.
# versão 27.py Diálogos Finais e Efeito de Degradê (Derrota do Chefe). 1 hora.
# versão 28.py Caminhada automática, salto e tela preta. 1 hora.
# versão 29.py Tela de Vitória Final e Retorno ao Menu Principal. 1 hora.
...
while True:
    ...
    # Versão 25/26 - Lógica da Luta Final
        ...
        img_atual_boss = img_boss # < --------------------------------------------------- V 25.3
        ...
        if status_boss == "MOVENDO":
            ...
            img_atual_boss = img_boss # < --------------------------------------------------- V 25.3
        # < ---------------------------------------------------------------------- V 25.3
        # --- O GOLPE INDEFENSÁVEL (Corrigido) ---
        elif status_boss == "ATACANDO":
            # 1. Escolha do Sprite do Boss no ataque
            if timer_boss < 15:
                img_atual_boss = img_boss_ataque_1 # Punho erguido
            else:
                img_atual_boss = img_boss_ataque_2 # Energia Negra
            
            # 2. Desenho da explosão FIXA NO CHÃO
            # Usamos 'altura_do_chao' em vez de 'y_lufy'
            tela.blit(luffy_explosao, (x_lufy - 20, altura_do_chao)) 
            
            # 3. Dano apenas se estiver no chão
            if not luffy_no_ar and timer_boss == 1:
                vidas_char -= 1
                x_lufy -= 50 
            
            if timer_boss >= 45: # Aumentei um pouco o tempo para ver a energia negra
                status_boss = "MOVENDO"
                timer_boss = 0
        
        else:
            # Se não estiver atacando, usa o sprite normal
            img_atual_boss = img_boss
        # < ---------------------------------------------------------------------- V 25.3

        # 2. CONTROLES E ATAQUE DO LUFFY
        ...
        # 3. DESENHO E DANO NO BOSS
        if not (status_boss == "PISCANDO" and (timer_boss % 10 < 5)):
            tela.blit(img_boss, (x_boss, y_boss))
        
        # Gerencia Sprite de Ataque ou Normal
        if atacando:
            timer_anim_ataque += 1
            if timer_anim_ataque >= VELOCIDADE_ANIMACAO:
                timer_anim_ataque = 0
            ...
            if indice_anim_ataque < len(animacao_ataque):
                luffy_atual = animacao_ataque[indice_anim_ataque]
                if indice_anim_ataque == 3: # Frame do impacto
                    r_golpe = pygame.Rect(x_lufy + 100, y_lufy, 150, 100)
                    r_boss = pygame.Rect(x_boss, y_boss, 80, 120)
                    if r_golpe.colliderect(r_boss) and status_boss != "ATACANDO":
                        vidas_boss -= 1
                        x_boss += 30 # Empurrão
                        atacando = False
            else: atacando = False
        else:
            luffy_atual = char_1 if not luffy_no_ar else char_corrida
        ...
        for i in range(vidas_char):
            tela.blit(img_coracao, (30 + (i * 35), 30))
        txt_vboss = fonte.render(f"BOSS HP: {vidas_boss}", True, (255, 0, 0))
        tela.blit(txt_vboss, (largura - 200, 30))

        if vidas_boss <= 0: status_jogo = "DIGITANDO_NOME" # Temporário até a V.29
        if vidas_char <= 0: status_jogo = "DIGITANDO_NOME"; resetar_jogo()
        # No final da lógica BOSS_LUTA:
        if not (status_boss == "PISCANDO" and (timer_boss % 10 < 5)):
            # Agora desenha o sprite que a IA escolheu (Normal, Punho ou Energia)
