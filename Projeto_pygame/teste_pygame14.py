...
#
# SISTEMA SCORE NO MENU
exibir_scores = False
lista_scores = [] # Guardará dicionários: {"nome": "Luffy", "pontos": 100}
# Variável para o nome do jogador atual (pode ser fixo por enquanto ou um input)
nome_jogador_atual = "Luffy" 
...
while True:
    for event in pygame.event.get(): # percorre a lista de eventos do pygame
        ...
        if status_jogo == "MENU" and event.type == MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                # Botão INICIAR (Ajuste os valores conforme sua imagem)
                # quanto a tela tem mesmo? largura, altura = 800,600 # janela do jogo (x,y)
#                if 450 < posicao_mouse[0] < 750 and 280 < posicao_mouse[1] < 360: # tenta se localizar pela (x,y)
    ...
    if status_jogo == "MENU":
        ...
        # Versao 14 (Se vc nao lembrar como explicar... foi pq google te ajudou aqui...)
        if exibir_scores:
            # Desenha um fundo semitransparente para a janela de score
            pygame.draw.rect(tela, (0, 0, 0), (50, 100, 400, 300)) # Janela no lado esquerdo
            pygame.draw.rect(tela, (255, 215, 0), (50, 100, 400, 300), 3) # Borda dourada
            
            titulo_score = fonte.render("TOP 3 PIRATAS", True, (255, 215, 0))
            tela.blit(titulo_score, (150, 120))

            # Loop para mostrar os scores
            for i, s in enumerate(lista_scores):
                texto = f"#{i+1} {s['nome']}: {s['pontos']}"
                img_texto = fonte.render(texto, True, (255, 255, 255))
                tela.blit(img_texto, (80, 180 + (i * 50))) 
        # Versao 14 - fim desse trecho
        ...
        if luffy_rect.colliderect(barril_rect): # Verifica se os retângulos colidem 
            ...
            if vidas_char <= 0:
                # VERSAO 14 - NOVO
                novo_score = {"nome": nome_jogador_atual, "pontos": pontos}
