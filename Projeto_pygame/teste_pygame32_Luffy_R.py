...
# Carrega as músicas (Certifique-se de que os nomes dos arquivos estão corretos)
# musica_fase = "music/Luffy Fierce Attack.mp3"
# musica_boss = "music/The Very Very Very Strongest - Epic Version.mp3"
musica_fase = resource_path("music/Luffy Fierce Attack.mp3")
musica_boss = resource_path("music/The Very Very Very Strongest - Epic Version.mp3")

# Começa tocando a música da fase em loop
pygame.mixer.music.load(musica_fase)
pygame.mixer.music.set_volume(0.4) # Volume de 0 a 1
pygame.mixer.music.play(-1)

# Carregue os arquivos (use o resource_path para o executável)
som_pulo = pygame.mixer.Sound(resource_path("music/gomu-gomu-no.mp3"))
som_explosao = pygame.mixer.Sound(resource_path("music/som_explosao_1.ogg"))
...
imagem_menu = pygame.image.load(resource_path("image/menu_jogo.png"))
imagem_menu = pygame.transform.scale(imagem_menu, (largura, altura))

wallpaper = pygame.image.load(resource_path("image/background_lufy_022.png"))  # carrega a imagem do cenário do jogo
wallpaper = pygame.transform.scale(wallpaper, (largura, altura)) # redimensiona

corrimao = pygame.image.load(resource_path("image/background_lufy_023.png"))
corrimao = pygame.transform.scale(corrimao, (largura, 200))

quadro_ranking = pygame.image.load(resource_path("image/menu_jogo4.png")) # Nova imagem para score
quadro_ranking = pygame.transform.scale(quadro_ranking, (350, 250)) # Tamanho x,y

quadro_comandos = pygame.image.load(resource_path("image/menu_jogo7.png")) # Nome do seu arquivo
quadro_comandos = pygame.transform.scale(quadro_comandos, (350, 250)) # Mesmo tamanho do ranking

img_fruta = pygame.image.load(resource_path("sprites/demon_fruits (1).png"))
img_fruta = pygame.transform.scale(img_fruta, (40, 40))

tela_vitoria_img = pygame.image.load(resource_path("image/tela_vitoria1.png")) # < --- V 28
...
char_1 = pygame.image.load(resource_path("sprites/luffy_sprite_5002.png")) # Carrega o sprite do character
char_1 = pygame.transform.scale(char_1, (largura_personagem, altura_personagem)) # Redimensiona o sprite
char_corrida = pygame.image.load(resource_path("sprites/luffy_sprite_5003.png")) # Versao 17 
...
barril_01 = pygame.image.load(resource_path("sprites/barril_01.png"))
barril_01 = pygame.transform.scale(barril_01, (largura_barril, altura_barril))

img_coracao = pygame.image.load(resource_path("sprites/heart_1_short.png")) # Versao 16
img_coracao = pygame.transform.scale(img_coracao, (30, 30)) # Versao 16

largura_triplo, altura_triplo = 150, 150
img_barril_triplo = pygame.image.load(resource_path("sprites/barril_02.png")) # V. 19
...
luffy_energia = pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5026.png")), (80, 60)) # Golpe a frente
luffy_explosao = pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5020.png")), (100, 100))
animacao_ataque = [
    pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5011.png")), (largura_personagem, altura_personagem)),
    pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5012.png")), (largura_personagem + 20, altura_personagem)),
    pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5015.png")), (120, 100)), # Braço esticando
    pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5016.png")), (120, 100))  # Golpe final
]
luffy_energia = pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5026.png")), (150, 60))
luffy_explosao = pygame.transform.scale(pygame.image.load(resource_path("sprites/luffy_sprite_5020.png")), (100, 100))

# Versão 24
# Imagem do cenário do Boss
wallpaper_boss = pygame.transform.scale(pygame.image.load(resource_path("image/background_lufy_012.png")), (largura, altura))
# Sprite do Chefe 
img_boss = pygame.transform.scale(pygame.image.load(resource_path("sprites/z_almirante_ancestral_2002.png")), (80, 120))
img_boss_ataque_1 = pygame.image.load(resource_path("sprites/z_almirante_ancestral_2003.png")) # < ----- V 25.3
img_boss_ataque_1 = pygame.transform.scale(img_boss_ataque_1, (80, 120)) # < ------------ V 25.3
img_boss_ataque_2 = pygame.image.load(resource_path("sprites/z_almirante_ancestral_2006.png")) # < ----- V 25.3
...
# Versão 28
timer_tela_final = 180 # 3 segundos (60 FPS * 3)
...
def resetar_jogo(): # V. 17
    global vidas_char, x_barril, velocidade_jogo_barril, x_lufy, y_lufy, delay_inicio
    global luffy_atual, luffy_no_ar, frutas_coletadas, proximo_alvo_fruta, tempo_fase
    global metas_batidas, timer_vitoria_fase, nome_temp,  vidas_boss, status_boss, x_boss, timer_boss
    # vidas_char = 3
    vidas_char = 5
    x_barril = largura
    ...
    nome_temp = "" # Limpa o nome temporário para o próximo jogador
...
# versão 29.py Finalização das instruções de controle do jogo via menu. 1 hora.
# versão 30.py Inserção da música. 1 hora.
# Nesse ponto o nosso MVP (Mínimo Produto Viável) está finalizado.
# O jogo é jogavel, e é relevante o suficiente.
# Porém, tive problemas ao gerar o executavel. Vou tentar corrigir isso na proxima versão.
# versão 31.py 5 horas.
# versão 32.py Gerando o executavel. Arquivos. 2 horas.
...
while True:
    for event in pygame.event.get(): # percorre a lista de eventos do pygame
        ...
        # 2. EVENTOS AO DIGITAR NOME
            ...
            if event.type == KEYDOWN:
                ...
                #     status_jogo = "MENU"
                # < -------------------------------------------------------------------------- V. 28
                if event.key == K_RETURN: # Apertou Enter, salva
                    if nome_temp == "": nome_temp = "Pirata Anonimo"
                    novo_score = {"nome": nome_temp, "pontos": pontos}
                    lista_scores.append(novo_score)
                    lista_scores = sorted(lista_scores, key=lambda x: x['pontos'], reverse=True)[:3]
                    
                    # --- NOVO: LÓGICA DA TELA DE VITÓRIA ---
                    if vidas_boss <= 0:
                        status_jogo = "MOSTRAR_VITORIA"
                        timer_tela_final = 180 # 3 segundos
                    else:
                        status_jogo = "MENU"
                # < -------------------------------------------------------------------------- V. 28
    ...
    if status_jogo == "MENU":
        ...
        if exibir_scores:
            ...
            # 2. Desenha a Janela de Comandos (Quando o ranking estiver oculto)
            # Certifique-se de que carregou 'quadro_comandos' na seção 4 do seu código!
            tela.blit(quadro_comandos, (50, 250)) 
    ...
    # VERSAO 15
        ...
        if x_barril < -150:
            x_barril = largura
            pontos += 1
            barril_atual = random.choice(["simples", "triplo"]) # É aqui que define se o próximo barril é simples ou triplo
        ...
        if atacando and indice_anim_ataque == 3:
            if rect_golpe.colliderect(barril_rect):
                ...
                som_explosao.play() # Toca o som da explosão # Tirei pq deu problema # Na verdade nao deu erro
    ...
    # V. 24 - BOSS
        ...
        # < ---------------------------------------------------------------------------------- V. 29 MUSICA
        # --- TROCA PARA MÚSICA DO CHEFE ---
        # Verificamos se a música atual é diferente da do boss para não carregar toda hora
        if pygame.mixer.music.get_busy() and status_boss == "MOVENDO": # Pequena trava lógica
             # Se você quiser que a música mude EXATAMENTE na entrada:
             # pygame.mixer.music.stop(2000)
             pygame.mixer.music.fadeout(2000) 
             pygame.mixer.music.load(musica_boss)
             pygame.mixer.music.play(-1)
             status_boss = "CARREGANDO_BOSS" # Mudamos o status só para a música não carregar de novo no próximo frame
        # < ---------------------------------------------------------------------------------- V. 29 MUSICA
        ...
        if x_lufy >= 150 and x_boss <= 550:
            ...
            if timer_vitoria_fase > -120: # Reutilizando o timer para economizar var
                ...
                timer_boss = 0
                status_boss = "MOVENDO" # < ------------------------------------ Faz boss voltar a mexer
    ...
    # Versão 25/26 - Lógica da Luta Final
        ...
        # (V.32)
        tempo_fase -= 1 # Continua a contagem de tempo < ----------------- (V.32) Tinha esquecido
        if tempo_fase <= 0:
            status_jogo = "DIGITANDO_NOME"
            # NÃO chame resetar_jogo() aqui para não zerar os pontos antes da hora!
        # (V.32)
        tela.blit(wallpaper_boss, (0, 0))
        timer_boss += 1
        keys = pygame.key.get_pressed()
...
#            if x_boss <= 400 or x_boss >= largura - 100: vel_boss_x *= -1
            if x_boss <= 400 or x_boss >= largura - 50: vel_boss_x *= -1 # Versão 31 - Ajuste

            # # Versão 31 - Ajuste de limites para o movimento do Boss
            # # --- ADICIONE ESTA TRAVA DE SEGURANÇA AQUI ---
            # if x_boss > largura - 100: x_boss = largura - 100 # Não deixa fugir pela direita
            # if x_boss < 400: x_boss = 400                    # Não deixa fugir pela esquerda
            # # ---------------------------------------------
            # # Versão 31
            ...
            if indice_anim_ataque < len(animacao_ataque):
                ...
                if indice_anim_ataque == 3:
                    ...
                    if rect_golpe.colliderect(rect_boss):
                        ...
                        # x_boss += 30 # Quando o Luffy acertar Empurra o chefe X pixels para trás
                        # Empurra apenas se não for sair da tela
                        if x_boss + 30 < largura - 100: # < -------- V. 31 - Ajuste para não deixar o Boss fugir da tela
                            x_boss += 30 # < ----------------------- V. 31
                        som_explosao.play()
                        atacando = False 
            else:
        ...
        if vidas_boss <= 0 and status_boss != "DERROTADO":
            pygame.mixer.music.stop() # Para a música do Boss na hora!
            # Opcional: Se tiver uma música de vitória curta:
            # pygame.mixer.music.load("music/vitoria.mp3")
            # pygame.mixer.music.play()
            pontos += 1000 # Bônus por derrotar o Almirante
            # status_jogo = "VITORIA_FINAL"
            timer_vitoria_fase = 300 # Tempo da cena final
            status_boss = "DERROTADO" # < ---------------------------------------- V. 32
        ...
        # --- V. 32 
        if status_boss == "DERROTADO":
            if not (status_boss == "PISCANDO" and (timer_boss % 10 < 5)):
                tela.blit(img_atual_boss, (x_boss, y_boss))
            # 1. O Chefe para de ser desenhado (ele some)
            # 2. Criamos o efeito de escurecer (Fade)
            
            # Usamos o timer_vitoria_fase que você já tem (300 frames = 5 segundos)
            # Quanto menor o timer, mais escuro fica.
            alpha_value = 255 - (timer_vitoria_fase * 0.85) # Cálculo para chegar em 255 (preto total)
            if alpha_value > 255: alpha_value = 255
            if alpha_value < 0: alpha_value = 0

            # Desenha o retângulo preto por cima de tudo
            ...
            # Quando ficar tudo preto, aí sim mudamos para a tela de mensagens
            if timer_vitoria_fase <= 0:
                status_jogo = "VITORIA_FINAL"
                timer_vitoria_fase = 300 # Reseta para o tempo da tela final
        # V. 32 - Fim da lógica de derrota do Boss e transição para a tela de vitória

        # EXIBE O TEMPO NO BOSS (Adicione isso no final do bloco BOSS_LUTA)
        segundos = tempo_fase // 60
        cor_relogio = (255, 255, 255)
        if segundos <= 10: cor_relogio = (255, 0, 0)
        txt_tempo = fonte.render(f"Tempo: {segundos}s", True, cor_relogio)
        tela.blit(txt_tempo, (largura // 2 - 50, 30))
    ...
    # --- NOVO: TELA ESPECIAL DE 3 SEGUNDOS (V.28) ---
    elif status_jogo == "MOSTRAR_VITORIA":
        tela.blit(tela_vitoria_img, (0, 0)) # Aquela imagem tela_vitoria1.png
        timer_tela_final -= 1
        
        if timer_tela_final <= 0:
            # --- REINICIANDO A MÚSICA AQUI ---
            pygame.mixer.music.fadeout(1000) # Para a música do boss suavemente
            pygame.mixer.music.load(musica_fase)
            pygame.mixer.music.play(-1)
            status_jogo = "MENU"
            resetar_jogo()
...
# parabens por chegar aqui
# all the best. renan.fsc@outlook.com