...
while True:
    ...
    # Versão 25 - Comandos e lógica BOSS "IA"
        ...
        if status_boss == "MOVENDO":
            ...
            # Limites da tela para o Boss não fugir
            if x_boss <= 300 or x_boss >= largura - 150: vel_boss_x *= -1
            if y_boss <= 200 or y_boss >= altura - 200: vel_boss_y *= -1
            
            # A cada 3 segundos (180 frames), ele decide piscar para atacar
            if timer_boss >= 180:
                status_boss = "PISCANDO"
                timer_boss = 0

        # --- AVISO DE ATAQUE (PISCANDO) ---
        elif status_boss == "PISCANDO":
            # Faz o boss sumir e aparecer (piscar)
            if timer_boss % 10 < 5:
                tela.blit(img_boss, (x_boss, y_boss))
            
            if timer_boss >= 60: # Pisca por 1 segundo
                status_boss = "ATACANDO"
                timer_boss = 0

        # --- O GOLPE INDEFENSÁVEL ---
        elif status_boss == "ATACANDO":
            # Desenha um efeito visual do golpe (pode usar o sprite 5020 de explosão)
            tela.blit(luffy_explosao, (x_lufy, y_lufy)) 
            
            # Se o Luffy não estiver pulando, ele leva dano
            if not luffy_no_ar:
                vidas_char -= 1
                # Pequeno recuo do Luffy ao levar dano
                x_lufy -= 50 
            
            if timer_boss >= 30: # Golpe dura meio segundo
                status_boss = "MOVENDO"
                timer_boss = 0

        # --- CONTROLE DO JOGADOR NA LUTA ---
        keys = pygame.key.get_pressed()
        if keys[K_w] and not luffy_no_ar:
            luffy_no_ar = True
            luffy_vel_y = -15
        
        # Movimentação horizontal do Luffy para desviar
        ...
        if status_boss != "PISCANDO" or (timer_boss % 10 < 5):
            tela.blit(img_boss, (x_boss, y_boss))
        tela.blit(char_1 if not luffy_no_ar else char_corrida, (x_lufy, y_lufy))
