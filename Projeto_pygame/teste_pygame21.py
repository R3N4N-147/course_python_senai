...
# Variável para controlar se mostramos a explosão - Versão 21
...
while True:
    ...
    # VERSAO 15
        ...
        if delay_inicio > 0:
            ...
            # V. 19 - Nova lógica do movimento do barril
            x_barril -= 7 + velocidade_jogo_barril 
            if x_barril < -150: # Aumentei o limite para o barril triplo sair todo da tela
                x_barril = largura
                pontos += 1
                
                # SORTEIO DO PRÓXIMO BARRIL
                if random.randint(0, 1) == 0:
                    barril_atual = "simples"
                else:
                    barril_atual = "triplo"

                if pontos % 20 == 0: # Sincronizado com a marca de frutas
                    velocidade_jogo_barril += 1
            # V.19
...
#            if pontos > 0 and pontos % 20 == 0 and not fruta_ativa:
        ...
        # tela.blit(barril_01, (x_barril, altura_do_chao)) # Desenha o barril # blit (x,y) # Sem uso da V.19
        ...
        # Captura do comando de ataque - V. 20
        if keys[K_SPACE] and not atacando:
            atacando = True
            duracao_ataque = 15 # O ataque dura 15 frames (~0.25s)
        if atacando:
            duracao_ataque -= 1
            if duracao_ataque <= 0:
                atacando = False
        ...
        if barril_atual == "simples":
            ...
            barril_rect = pygame.Rect(x_barril, altura_do_chao, largura_barril-5, altura_barril-5)
        elif barril_atual == "triplo":
            tela.blit(img_barril_triplo, (x_barril, altura_do_chao))
            # O retângulo de colisão do triplo é maior!
            barril_rect = pygame.Rect(x_barril, altura_do_chao, largura_triplo-10, altura_triplo-10)
        # NOVA COLISAO - V. 19

        # NOVA - lógica da colisão - V. 20
        # NOVA - lógica da colisão - V. 21
        if luffy_rect.colliderect(barril_rect):
            # if atacando:
            #     # DESTRUIU O BARRIL!
            #     x_barril = largura # Reseta o barril
            #     if barril_atual == "triplo":
            #         pontos += 15 # Pontuação tripla (5x3)
            #     else:
            #         pontos += 5  # Pontuação normal
                
            #     # Sorteia o próximo barril imediatamente
            #     barril_atual = "simples" if random.randint(0, 1) == 0 else "triplo"
            if atacando:
                # 1. Escolhe o sprite de ataque
                luffy_atual = luffy_ataque_corpo
                
                # 2. Cria o retângulo da ENERGIA (mais à frente do Luffy)
                # Ele fica 50 pixels a frente do x do personagem
                rect_energia = pygame.Rect(x_lufy + 60, y_lufy + 20, 80, 60)
                
                # 3. Desenha o efeito de energia
                tela.blit(luffy_energia, (x_lufy + 60, y_lufy + 20))

                # 4. Checa se a ENERGIA tocou no barril
                if rect_energia.colliderect(barril_rect):
                    # ATINGIU!
                    exibindo_explosao = True
                    timer_explosao = 10 # Mostra a explosão por 10 frames
                    x_explosao, y_explosao = x_barril, altura_do_chao # Salva onde foi o impacto
                    
                    # Reseta o barril e soma pontos (conforme lógica anterior)
                    x_barril = largura
                    pontos += 15 if barril_atual == "triplo" else 5
                    barril_atual = random.choice(["simples", "triplo"])
                    atacando = False # O golpe termina ao impactar
                # NOVA - lógica da colisão - V. 21
            else:
                # NÃO ESTAVA ATACANDO: LEVOU DANO
